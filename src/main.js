...
function sortGalleryItems(method) {
    galleryItems.sort((a, b) => {
        switch(method) {
            case 'newest':
          ***REMOVED*** new Date(b.timestamp) - new Date(a.timestamp);
            case 'oldest':
          ***REMOVED*** new Date(a.timestamp) - new Date(b.timestamp);
            case 'name':
          ***REMOVED*** (a.metadata?.name || '').localeCompare(b.metadata?.name || '');
            default:
          ***REMOVED*** 0;
        }
    });
...
function filterGalleryItems(searchTerm) {
  ...
  ***REMOVED*** name.includes(searchTerm.toLowerCase());
    });
}
